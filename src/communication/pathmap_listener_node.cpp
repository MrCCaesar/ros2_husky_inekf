...
class PathMapListenerNode : public rclcpp::Node
{
public:
  PathMapListenerNode() : Node("path_map_listener_node")
  {
    // 声明参数
    this->declare_parameter("output_file", "/tmp/pathmap_tum.txt");
    this->declare_parameter("path_topic", "zed_node/path_map");
    ...
    path_sub_ = this->create_subscription<nav_msgs::msg::Path>(
        path_topic_, 1000,
        std::bind(&PathMapListenerNode::pathMapCallback, this, std::placeholders::_1));

    RCLCPP_INFO(this->get_logger(), "Path Map Listener Node initialized");
    RCLCPP_INFO(this->get_logger(), "Subscribing to: %s", path_topic_.c_str());
...
private:
  ...
  void cameraOdomCallback(const nav_msgs::msg::Odometry::SharedPtr msg)
  {
    ...
    RCLCPP_DEBUG(this->get_logger(),
                 "Received camera odom data: [%.3f, %.3f, %.3f]",
                 position.x, position.y, position.z);
...
int main(int argc, char **argv)
{
  ...
  auto node = std::make_shared<PathMapListenerNode>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}