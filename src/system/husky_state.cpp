...
namespace husky_inekf
{
        ...
        const std::shared_ptr<husky_inekf::JointStateMeasurement> next_joint_state)
    {
        ...
        q_.block<4, 1>(6, 0) = joint_state_data.get()->getJointPosition();  // wheel position
        dq_.block<4, 1>(6, 0) = joint_state_data.get()->getJointVelocity(); // wheel velocity
    ...
    void HuskyState::clear()
    {
        ...
        imu_bias_ = Eigen::Matrix<double, 6, 1>::Zero();
