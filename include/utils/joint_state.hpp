...
#include "sensor_msgs/msg/joint_state.hpp" // ROS2 message header
...
namespace husky_inekf
{
    ...
    public:
        ...
        JointStateMeasurement(const sensor_msgs::msg::JointState &joint_msg,
                              unsigned int ENCODER_DIM) : encoder_dim_(ENCODER_DIM)
        {

            wheel_radius_ = 0.15;
            vehicle_track_width_ = 0.412;
            vehicle_length_ = 0.0;
            type_ = JOINT_STATE;
        ...
        {
            for (unsigned int i = 0; i < encoder_dim_; ++i)
            {
        ...
        void setBodyVelocity()
        {
            ...
            body_ang_vel_(2) = (vr - vl) / vehicle_track_width_ *
                               std::pow(std::cos(std::atan2(vehicle_length_, vehicle_track_width_)), 2); // [dx dy dz]
        }
